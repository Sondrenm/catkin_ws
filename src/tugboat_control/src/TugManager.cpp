...
{
  if (distress > MUCH_DISTRESS * (tugRequests.size() + 1) && waypTugs.data.size() > tugRequests.size() ) {
    ...
    if(add_client->call(*add_srv)){
      ...
      std::cout << "Request approved. " << tugRequests.size() << " tugs on their way to support ship\n";
    }
    else 
    {
      std::cout << "Request denied\n";
    }
  } else if(distress < MUCH_DISTRESS * (tugRequests.size() - 1)) {
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    { //All tugboats added to control
      ...
      tempRequest.ID = ++orderIDcount;
      tugRequests.push_back(tempRequest);
      std::cout << "Request approved. " << tugRequests.size() << " tugs shipbound\n";
      }
      else 
      {
        std::cout << "Request denied\n";
